...
onto the next generation, with more aggressive plants having more dominant genes. The ultimate choice of each gene will be random but the higher percentage a plant has in distribution the more likely it has more of an effect on the next generations genes.
...
When the simulation has reached its conclusion either by reaching the specified time given by the user, being forcibly stopped by the user or by the simulation reaching an endpoint due to lack of plants on the board. The users will be taken to the end screen with a timeline showing the overall timeline of the simulation which can be manipulated by the user. They will be given the option to return to the home screen.

**2.2 User Characteristics and Objectives**
We expect users to be knowledgeable about a Linux operating system of their choice and how to execute a basic .jar file. 

The user will be expected to be prompted by a newly opened window in their browser that they have set as default. The user can then select to either start a new simulation or load up and continue on from an old one. If the user chooses to start a new simulation they can set such variables such as the abundance of resources the plants need to survive, the size of the simulation space, the number of plants that the simulation starts off with and how long the simulation will run for. 
...
Dependencies with other requirements
This requirement will depend on any plant related objectives, such as how the plants grow, what they need at the current time and all of their various attributes. This requirement will also depend on the dynamic environment simulation as the way the plants decide to interact with each other, especially when deciding to compete over resources, will depend on what resources available.
...
**3.4 Single Generation Simulation**
Description
This refers to the simulation running to completion by not having the plants reproduce, meaning the simulation will end once the first generation has died off. The plants that the simulation starts off with, will grow towards their goals as expected just they won’t reproduce when they reach maturity and instead will just die off.
Criticality
If we can get the simulation working correctly for at least 1 generation, we would have built a solid enough foundation to build up the rest of the features on.
Technical issues
	Memory consumption would be the main technical issue. Depending on how many calculations each plant has to make within a certain time frame, along with the simulation environment updating its own state constantly, memory consumption can be a very real problem depending on the efficiency of our algorithms and the number of plants.
Dependencies with other requirements
Since the simulation will be running at this point, it is dependent on the plant’s activity, the plants interactions with each other and the dynamic environment present in the simulation. This is also dependent on how well we maximise our efficiency in our code as the more efficient we are, the less memory and time the simulation will use to run itself. 
...
Criticality
This is an important part of the simulation as a whole but is very heavily dependant on the other parts working before it can be achieved. We need to have implemented as a part of simulation to lay the ground for a multi generational environment.
Technical issues
The possible issues with this requirement is to make sure that the alterations of the plants genes are within reason and allow the plant the chance of growth and survival in the next generation. Another issue is to regulate the consistency of the plants mutations and make sure it happens enough to be realistic but not too overbearing on the alterations of the genes as a whole.
...
**3.6 Multi Generational Simulation**
Description
...
Time for calculation is the last issue that could arise. Similar to the memory availability, the more plants that’ll populate our simulation space, the more plant activity calculations the simulation will have to do simultaneously. This can lead to the simulation slowing down as it has to take time to do the calculations. We hope to minimize this by writing as efficient algorithms as we can.
Dependencies with other requirements
Due to the complexity of running a multi-generational simulation, this requirement depends on everything that is written above it. Without the foundation the requirements above this one in the list provides, we can’t run it at all.

**3.7 UI/GUI Functionality**
Description
...
Dependencies with other requirements
The UI and GUI are heavily dependant on the rest of the project as without the other functions there would be nothing to build a UI for and nothing to build a GUI on. The UI is less dependant on the simulation as a whole but the GUI is very dependent on having a simulation to graph
...
**6.1 **
Work on the Simulation Function 
Start and End Date to work on it to work on it
07/12/19-14/02/20
Major Objectives
To get the simulation to run within our defined constraints
Software Requirements
Java
Java Simulation Library DESMO-J
**6.2**
Work on the UI Function 
Start and End Date
14/02/20-06/03/20
Major Objectives
To give the simulation a refined live updating UI
Software Requirements
Javascript
Javascript Library 3.js
**6.3**
User Manual 
Start and End Date to work on it
21/02/20-06/03/20
Major Objectives
...
21/02/20-06/03/20
Major Objectives
To show a demonstration of how use our program
Software Requirements
